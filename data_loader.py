        ...
        for index in range(index0, index0 + len(self.data_lists)):
            try:
                ...
                if self.opts.data_format == 3:
                    ...
                    path = os.path.join(self.opts.data_dir.replace('125k_data2','125k_new'), data_file_name_new)
...
def lora_loader(opts):
    ...
    if opts.data_format == 1:
        ...
        vals_35 = set([i.split('_')[0] for i in os.listdir(os.path.join(opts.data_dir.replace('_data2','_new')))])
