...
class maskCNNModel3(nn.Module):
    ...
    def forward(self, xs):
        ...
        for i in range(self.opts.layer_cnt):
            ...
            #outmax *= 0
            #outmin *= 0
            #self.save_samples(outs, self.opts, str(i)+'outs')
            #self.save_samples([outavg,outmax,outmin], self.opts, str(i)+'avgs')
            '''
            ...
            for idx in range(len(xs)):
                ...
                if idx>-1:continue
                '''
                for k in range(len(self.conv2[i])):
                    print(type(self.conv2[i][k]))
                    if not isinstance(self.conv2[i][k], nn.Conv2d): continue
                    sources = self.conv2[i][k].weight.data
                    print(k,sources.shape)
                    continue'''
                sources = to_data(self.conv2[i][0].weight.data)
                print(sources.shape)
                print('orig ',np.mean(sources[:,:256]))
                print('max  ',np.mean(sources[:,256:512]))
                print('min  ' ,np.mean(sources[:,512:768]))
                print('input',np.mean(sources[:,768:]))
                for k in range(4,5):

                    sources = self.conv2[i][k].weight.data.cpu()
                    b, c, h, w = sources.shape
                    print(sources.shape)
                    merged = np.zeros([b * (h+1), c * (w+1)])
                    for ii in range(b):
                        for j in range(c):
                            merged[ii * (h+1):(ii + 1) * (h+1)-1, j * (w+1): (j + 1) * (w+1)-1] = sources[ii, j]
                    #merged = to_data(sources.squeeze())
                    path = os.path.join(self.opts.checkpoint_dir, 'weights-'+str(i)+'-'+str(k)+'.png')
                    merged = (merged - np.amin(merged)) / (np.amax(merged) - np.amin(merged)) * 255
                    #merged = cv2.flip(merged, 0)
                    cv2.imwrite(path, merged)
                    print('SAMPLE: {}'.format(path))
